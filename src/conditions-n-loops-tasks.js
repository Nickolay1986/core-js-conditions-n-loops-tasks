...
function convertToRomanNumerals(num) {
  ...
  for (let i = 0; i < romanNums.length; i += 1) {
    const [arabianNum, romanNum] = romanNums[i];
    while (number >= arabianNum) {
      result += romanNum;
      number -= arabianNum;
    }
  }
...
function rotateMatrix(matrix) {
  const mtxCopy = [...matrix];
  const n = matrix.length;
  for (let i = 0; i < Math.floor(n / 2); i += 1) {
    for (let j = i; j < n - i - 1; j += 1) {
      const temp = mtxCopy[i][j];
      mtxCopy[i][j] = mtxCopy[n - j - 1][i];
      mtxCopy[n - j - 1][i] = mtxCopy[n - i - 1][n - j - 1];
      mtxCopy[n - i - 1][n - j - 1] = mtxCopy[j][n - i - 1];
      mtxCopy[j][n - i - 1] = temp;
    }
  }
  return mtxCopy;
}
...
function sortByAsc(arr) {
  ...
  const result = [...leftPart, pivot, ...rightPart];
  while (j < result.length) {
    arrSorted[j] = result[j];
    j += 1;
  }
  return arrSorted;
}
